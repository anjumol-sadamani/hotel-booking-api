const { isValidEmail, isValidDate, isValidRoomId } = require('../utils/validators');
...
exports.validateBooking = (req, res, next) => {
    ...
    if (!room_id || !email || !booking_date) {
        logger.warn('Booking attempt with missing fields', { room_id, email, booking_date });
    ...
    if (!isValidDate(booking_date)) {
        ...
        return res.status(400).json({ error: "Invalid date. Must be YYYY-MM-DD and not in the past" });
    }
