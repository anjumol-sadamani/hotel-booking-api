...
describe('RoomService', () => {
  ...
  describe('getAllRooms', () => {
    ...
    it('should handle errors when fetching all rooms', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error fetching rooms', { error: 'Database error' });
    });
  });
  ...
  describe('getRoomById', () => {
    ...
    it('should throw an error when room is not found', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error fetching room', { error: 'Room not found', roomId: 999 });
    });
    ...
    it('should handle database errors when fetching a room', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error fetching room', { error: 'Database error', roomId: 1 });
    });
  });
});