...
describe('BookingService', () => {
  ...
  describe('createBooking', () => {
    it('should create a booking successfully', async () => {
      ...
      const result = await BookingService.createBooking(1, 'test@example.com', '2024-08-20');
    ...
    it('should throw an error if the room is not available', async () => {
      ...
      await expect(BookingService.createBooking(1, 'test@example.com', '2023-08-20'))
        .rejects.toThrow('Room is not available for the selected date');

      expect(logger.error).toHaveBeenCalledWith('Error in createBooking', expect.any(Object));
    });
